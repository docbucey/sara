...
namespace DisabilityMapper
{
    public partial class OfficeSuiteWindow : Window
    {
        ...
        private async void SendEmail_Click(object sender, RoutedEventArgs e)
        {
            ...
            {
                SaveStatus.Text = "Type: to@email.com | Subject | Body";
                return;
            }
            ...
            if (parts.Length < 3)
            {
                SaveStatus.Text = "Format: to@email.com | Subject | Body";
                return;
            }
            ...
            ResponseBox.Text = status == "PASS" ? "Email sent!" : $"Email failed: {inner?.Value<string>("error") ?? status}";
            StatusText.Text = "Done";
        }
        ...
        private async void CalendarList_Click(object sender, RoutedEventArgs e)
        {
            ...
            {
                ResponseBox.Text = inner?.Value<string>("error") ?? "No events or calendar not configured";
            }
            StatusText.Text = "Done";
        }
